...
- **Forward-only State Commit Gate**: mọi thay đổi mới trong `characters/`, `worldbuilding/`, hoặc `plot/` phải có Canon Diff được thay đổi trong cùng changeset và có checkbox `[x]`; CI dùng `scripts/state-commit-guard.py --base <base-ref>`. Guard này không hồi tố audit legacy state.
