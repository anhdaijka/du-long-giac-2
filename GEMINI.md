...
- **Legacy pre-rewrite material is not rewrite authority**: các chapter, review, canon diff và durable state được tạo trước đợt chia lại timeline/viết lại từ đầu được coi là **workflow-test drafts** dùng để bắt lỗi Agent. Chúng có thể được tham khảo như test fixture/lịch sử thử nghiệm, nhưng CẤM tự động dùng làm canon nền cho bản rewrite mới nếu Tác giả chưa tái phê duyệt.
- **Rewrite bootstrap priority**: khi Tác giả bắt đầu đợt rewrite timeline mới, ưu tiên `story_database.sqlite3` + Evidence Packet mới + các quyết định rewrite được Tác giả duyệt. Không được suy diễn rằng trạng thái `approved/canonized` của bản thử nghiệm cũ tự động carry forward sang continuity mới.
